...
Ecoute /head_lock (std_msgs/Bool) : true=VERROU (50deg), false=DEVERROU (75deg).
...
LOCK_DEG, UNLOCK_DEG = 50.0, 75.0   # 50=verrouille 75=deverrouille (calibre 2026-07-07, autorite roby_system.cpp)
...
class HeadLock(Node):
    def __init__(self):
        ...
        self._ensure_50hz()
        self.set_angle(LOCK_DEG)            # repos = verrouille (etat sur)
        self.create_subscription(Bool, "/head_lock", self.cb, 10)
        self.get_logger().info("head_lock pret. /head_lock: true=VERROU(50) false=DEVERROU(75). Etat=VERROU")
