...
# Rejet d'outliers : si le delta entre 2 lectures consecutives depasse ce seuil,
# c'est un pulseIn corrompu (pas un mouvement physique). A 50 Hz, 20 deg/cycle =
# 1000 deg/s — bien au-dessus de la vitesse max physique du bras.
MAX_STEP_DEG = 20.0

# Calibration : depuis le package share (installe par CMakeLists.txt)
CALIB_PATH = os.path.join(
    get_package_share_directory("roby_hardware"),
    "config",
...
JOINT_PARAMS = {
    1: {"gear_num": 16, "gear_den": 85, "inverted": True, "name": "joint_1"},
    2: {"gear_num": 15, "gear_den": 44, "inverted": False, "name": "joint_2"},
...
class MotorTracker:
    ...
    def update(self, raw_deg):
        ...
        if self.last_raw is None:
            ...
            # Rejet d'outliers AVANT integration : un saut > MAX_STEP_DEG entre
            # 2 reads consecutifs ne peut pas etre un mouvement physique reel,
            # c'est forcement un pulseIn corrompu cote Arduino esclave.
            if abs(diff) > MAX_STEP_DEG:
                self.outliers_count += 1
                return self.filtered()
            self.unwrapped += diff
